...
async def main() -> None:
    ...
    tg_task = asyncio.create_task(notifier.run(), name="telegram")
    ...
    for task in (scanner_task, bot_task, tg_task):
        task.cancel()
    await asyncio.gather(scanner_task, bot_task, tg_task, return_exceptions=True)
